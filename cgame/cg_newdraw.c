...
static void CG_DrawPlayerItem( rectDef_t *rect, float scale, qboolean draw2D) {
  ...
  vec3_t origin, angles;
	...
	value = 0; //cg.snap->ps.stats[STAT_HOLDABLE_ITEM];
	if ( value ) {
...
static void CG_DrawSelectedPlayerHead( rectDef_t *rect, qboolean draw2D, qboolean voice ) {
  ...
  if (ci) {
      ...
      CG_Draw3DModel( rect->x, rect->y, rect->w, rect->h, ci->headModel[ci->activeTier], ci->headSkin[ci->activeTier], origin, angles );
...
static void CG_DrawPlayerHealth(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
  ...
  int value;
	...
	value = ps->stats[powerLevelCurrent];
...
float CG_GetValue(int ownerDraw) {
  ...
  case CG_PLAYER_HEALTH:
		return ps->stats[powerLevelCurrent];
    break;
  case CG_RED_SCORE:
		return cgs.scores1;
    break;
...
qboolean CG_OwnerDrawVisible(int flags) {
	...
	if (flags & CG_SHOW_HEALTHCRITICAL) {
		if (cg.snap->ps.stats[powerLevelCurrent] < 25) {
			return qtrue;
		}
	}
