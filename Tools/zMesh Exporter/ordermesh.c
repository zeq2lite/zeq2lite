...
void OrderMesh( int input[][3], int output[][3], int numTris ) {
	...
	int totalStrips = 0;
	int strip[MAX_TRIS][3];			// could dump directly into 'output', but 
	...
	while ( sumStrippedTriangles < numTris ) {
		...
		if(debug) {
			printf( " strips %4d stripped %5d/%5d last strip length %4d\r", totalStrips, sumStrippedTriangles, numTris, strippedTriangles );
		}
	}
